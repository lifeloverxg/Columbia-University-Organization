...
class myask extends Control
{
    ...
    function ac_index()
    {
        $query = "SELECT id, tid, tidname, tid2, tid2name, uid, title, digest, reward, dateline, expiredtime,
                  solvetime, status, replies FROM `#@__ask`
                  WHERE uid='{$this->cfg_ml->M_ID}' ORDER BY dateline DESC";
    ...
    function ac_answer()
    {
        $query = "SELECT a.id,a.title,a.tid,a.tidname,a.tid2,a.tid2name,a.uid,m.userid
                 FROM `#@__askanswer` AS s 
                 LEFT JOIN `#@__ask` AS a ON s.askid = a.id 
                 LEFT JOIN`#@__member` AS m ON m.mid = a.uid 
                 WHERE s.uid = '{$this->cfg_ml->M_ID}'";
        $dlist = new datalistcp();
        $dlist->pageSize = 20;
        $dlist->SetParameter('ct',"myask");
